...
@SpringComponent
@VaadinSessionScope
public class ActiveBudgetsProvider extends BudgetsDataProvider {

    private final TransactionService transactionService;

    protected ActiveBudgetsProvider(BudgetService budgetService, UserContextHolder userContextHolder, TransactionService transactionService) {
    ...
    public Stream<Budget> fetchBudgets(long userId) {
        BudgetFilter budgetFilter = BudgetFilter.activeBudgetsFilter();
        budgetFilter.setWithCategories(true);

        Sort budgetSort = Sort.by(Sort.Direction.DESC, "isFavourite")
